function Transaction__payment() {
    ...
    `,
        `<div id="locat-delivery">
            ...
            <br>
                <input type="text" id="location" title="Please enter your location" placeholder="123/A Hau Giang Street, P11, Q6, TP.HCM" required>
        </div>
        <div class="error-message" id="error-locat-delivery"></div>
        <label for="prev_address">Bạn có muốn sử dụng địa chỉ tài khoản không</label>
...
<h3>Transaction Complete!</h3></div>`
    ...
    function saveDataToLocalStorage() {
        ...
        let ArrayBill = JSON.parse(localStorage.getItem('ArrayBill')) || [];
        if (paymentMethod) {
            Bill.method = paymentMethod.value;

            switch (Bill.method) {
                case 'Momo':
                    Bill.phone = document.getElementById('momo_phone').value;
                    Bill.name = document.getElementById('momo_name').value;
                    Bill.pin = document.getElementById('momo_pin').value;
                    break;
                case 'ATM':
                    Bill.cardNumber = document.getElementById('atm_card_number').value;
                    Bill.expiredDate = document.getElementById('atm_expired_date').value;
                    Bill.cvc = document.getElementById('atm_cvc').value;
                    Bill.phone = document.getElementById('atm_phone').value;
                    Bill.pin = document.getElementById('atm_pin').value;
                    Bill.name = document.getElementById('atm_name').value;
                    break;
                case 'Cash':
                    Bill.phone = document.getElementById('cash_phone').value;
                    Bill.name = document.getElementById('cash_name').value;
                    Bill.note = document.getElementById('cash_note').value;
                    break;
            }
        }
        if (currentIndex === 1) { // Trang nhập địa chỉ
            if (Bill.address) {
                let checkbox = document.getElementById('prev_address');
                checkbox.addEventListener('click', function () {
                    if (checkbox.checked) {
                        checkbox.innerText = Bill.address;
                    }
                    else {
                        checkbox.innerText = '';
                    }
            ...
            Bill.location = locationInput ? locationInput.value : '';
        }
        ArrayBill.push(Bill);
        localStorage.setItem('ArrayBill', JSON.stringify(ArrayBill));
        console.log('Dữ liệu đã được lưu:', ArrayBill);
    }

    function getPaymentInfoFromLocalStorage() {
        const storedPaymentInfo = localStorage.getItem('ArrayBill');
    ...
    function clearData() {
        ...
        if (currentIndex === 1) {
            ...
            if (currentIndex === 2) {
                delete ArrayBill[currentIndex].location;
                // const locationInput = document.getElementById('location');
            ...
            console.log('Dữ liệu còn lại:', ArrayBill[indexToDelete]);
            localStorage.setItem("ArrayBill", JSON.stringify("ArrayBill"));
        }
        ...
        function showInfo() {
            const storedPaymentInfo = localStorage.getItem('ArrayBill');
            const ArrayBill = storedPaymentInfo ? JSON.parse(storedPaymentInfo) : [];
